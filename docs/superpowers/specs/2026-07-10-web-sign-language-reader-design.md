...
- `mobile/src/components/signLanguageReader/GestureRecognizer.ts` (+ `GestureRecognizer.verify.ts`) → `shared/signLanguageReader/GestureRecognizer.ts` (+ verify script). Zero logic changes — the file has no React/React Native imports today, so this is a pure move.
...
Visual structure matches the dashboard's existing design system (CSS custom properties `var(--accent)`, `var(--bg)`, `var(--bgCard)`, `var(--border)`, card-based layout with `var(--radius)`), the same conventions already used by `/dashboard/learn` and `/dashboard/community` — not mobile's `Colors`/`Spacing`/glassmorphism theme, which is mobile-only.
