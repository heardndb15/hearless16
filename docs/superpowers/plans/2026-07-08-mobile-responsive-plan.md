...
- Known pre-existing baseline issue (confirmed before any task in this plan started, so it is NOT a regression to chase): `npm run build` compiles and type-checks successfully, but static export fails on `/community` with `Error: @supabase/ssr: Your project's URL and API key are required` — this environment's `landing/.env.local` has the Supabase keys commented out, so Supabase-dependent pages can't prerender locally. Treat a build as passing for this plan's purposes when it reaches `✓ Compiled successfully` and the type-check step with no NEW errors, even if the export step still fails on `/community` for this reason. Do not attempt to fix env/Supabase config as part of any task here — out of scope.
