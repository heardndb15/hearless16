...
14. Mobile language-switcher inactive-tab state (around lines 207-208, the sibling of item 3's active-tab styling — same ternary, different branch): `border: lang === l ? "1.5px solid var(--accent)" : "1px solid rgba(0, 0, 0,0.15)"` → replace the inactive branch with `"1px solid var(--border)"`. For the background, **do not** use `var(--chipBg)` as a literal reading of this item might suggest — that's the *active* branch's own value (see item 3's ternary), so using it for both branches collapses the selected/unselected visual distinction. Use `var(--bg)` instead: `background: lang === l ? "var(--chipBg)" : "#F8FBFF"` → replace `"#F8FBFF"` with `"var(--bg)"`. (`--bg`'s light value `#F4F7FB` is visually indistinguishable from the original `#F8FBFF`, and it flips correctly in dark mode while keeping `--chipBg` visually distinct for the active state.)
